...
def get_partner_details(firm_name, partneradd_lnk, context=None):
        ...
        if partneradd_lnk:
            ...
            result['country'] = partneradd_lnk.country_id.name or ''
        return result
    ...
    def generate_tracking_no(self, cr, uid, ids, context={}, error=True):
        ...
        try:
            ...
            if not tracking_ref:
                ...
                ### Recipient
                if type == 'out':
                    print "outtttttttttttttttttttttttttttttttttttttt"
                ...
                receipient = Address(cust_address.name or cust_address.name, cust_address.street, cust_address.city, cust_address.state_id.code or '', cust_address.zip, cust_address.country_id.code, cust_address.street2 or '', cust_address.phone or '', cust_address.email, cust_address.name,cust_address.country_id.name)
                ...
                if shippingresp_lnk.type.lower() == 'usps' and not ('usps_active' in context.keys()):
                    ...
                    for each_move_lines in move_lines:
                        print "move lines......",each_move_lines
...
#                            package = endicia.Package(shippingresp_lnk.name, round(weight*16 >= 1.0 and weight*16 or 1.0,1), endicia.Package.shapes[picking_data.container_usps], picking_data.length_usps, picking_data.width_usps, picking_data.height_usps,picking_data.name,250)
                        package = endicia.Package(shippingresp_lnk.name, int(round(shippingresp_lnk.picking_id.weight_package * 16)), endicia.Package.shapes[picking_data.container_usps], picking_data.length_usps, picking_data.width_usps, picking_data.height_usps,picking_data.name,250)
                        if receipient.country_code.lower() != 'us' and receipient.country_code.lower() != 'usa' and receipient.country_code.lower() != 'pr':
                            if context.get('no_international') and shippingresp_lnk.name != 'First Class Mail International':
                                return False
                            package = endicia.Package(shippingresp_lnk.name, int(round(shippingresp_lnk.picking_id.weight_package * 16)), endicia.Package.shapes[picking_data.container_usps], picking_data.length_usps, picking_data.width_usps, picking_data.height_usps,picking_data.name,picking_data.sale_id.amount_total)
#                                package = endicia.Package(shippingresp_lnk.name, int(round(weight*16 >= 1.0 and weight*16 or 1.0)), endicia.Package.shapes[picking_data.container_usps], picking_data.length_usps, picking_data.width_usps, picking_data.height_usps,picking_data.name,picking_data.sale_id.amount_total)
                        ...
                        if international_label:
                            sale_brw = shippingresp_lnk.picking_id and shippingresp_lnk.picking_id.sale_id
                            if sale_brw:
                               sale_order_lines = sale_brw.order_line
                               for each_move_lines in sale_order_lines:
                                   if each_move_lines.product_id and each_move_lines.product_id.product_tmpl_id.type == 'service':
                                       continue
                                   weight_net = each_move_lines.product_id.product_tmpl_id.weight_net and each_move_lines.product_id.product_tmpl_id.weight_net*35.274*each_move_lines.product_uom_qty >= 1.0 and each_move_lines.product_id.product_tmpl_id.weight_net*35.274*each_move_lines.product_uom_qty or 1.0
                                   value = round(each_move_lines.price_subtotal,2) if each_move_lines.price_subtotal<> 0 else 0.01
                                   customs.append(endicia.Customs(each_move_lines.product_id.name, int(each_move_lines.product_uom_qty), int(round(weight_net)),value,shipper.country_code))
                        try:
                            print "customs",customs
                            ship_endicia = self.pool.get('shipping.endicia').get_endicia_info(cr,uid,context)
                            print "ship_endiciaship_endiciaship_endicia",ship_endicia
                            ...
                            print "requestrequestrequestrequest",request
...
#                            self.pool.get('sale.order').write(cr,uid,shippingresp_lnk.picking_id.sale_id.id,{'tracking_no':tracking_numbers})
                            ...
                            picking_ids = stock_obj.search(cr ,uid ,[('state','in',['draft','assigned','confirmed']),('label_recvd','=',False),('carrier_tracking_ref','=',False)])
                            if picking_ids:
                                vals={
                                'shipping_type' : shippingresp_lnk.picking_id.shipping_type or False,
                                'weight_package' : shippingresp_lnk.picking_id.weight_package or False,
                                'shipping_account' : shippingresp_lnk.picking_id.shipping_account or False,
                                'container_usps' : shippingresp_lnk.picking_id.container_usps or False,
                                'size_usps' : shippingresp_lnk.picking_id.size_usps or False,
                                'use_shipping' : True,}

                                update=stock_obj.write(cr,uid,picking_ids,vals)
...
#            _logger.info("Net FedEx Charge: %s %s", rate_detail.ShipmentRateDetail.TotalNetFedExCharge.Currency,rate_detail.ShipmentRateDetail.TotalNetFedExCharge.Amount)
    ...
    _columns = {
        'carrier_tracking_ref': fields.char('Carrier Tracking Ref', size=320),
...
#                    shipper_address = stockpicking.sale_id and stockpicking.sale_id.shop_id.cust_address or False
                    if stockpicking.sale_id :
                        ...
                        print "shipper adress......",shipper_address
                    else:
