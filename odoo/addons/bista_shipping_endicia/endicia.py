...
class Package(object):
	...
	'First Class Package International Service' : 'FirstClassPackageInternationalService',
...
#packages = [Package(stockpicking_lnk.service_type_usps, round(weight * 16,1), stockpicking_lnk.length_usps, stockpicking_lnk.width_usps, stockpicking_lnk.height_usps, value=1000, require_signature=3, reference='a12302b') ]
    def __init__(self, mail_class, weight_oz, shape, length, width, height, description='', value=0, require_signature=0, reference='a12302b'):        
...
#        request.MailClass = packages[0].mail_class if to_country_code.upper() == 'US' else 'International'
...
#        request.DateAdvance = 1
        ...
        try:
            print "helooooooooooooooooooooooooo inside usps....",request
            reply = self.client.service.CalculatePostageRates(request)

            if reply.Status != 0:
#                raise osv.except_osv(_('Warning !'),_(reply.ErrorMessage))
#                raise EndiciaError(reply.ErrorMessage)
                raise osv.except_osv(_('Warning'), _("Cannot generate Shipping Quotes for an invalid Address"))
            logging.debug(reply)
...
#            raise Exception(message)
            raise osv.except_osv(_('Warning !'),_('%s'%(message)))
        else:
...
class LabelRequest(EndiciaRequest):    
    ...
    def _get_xml(self):        
        ...
        if self.label_type in ('Domestic','International'):
			if self.package.mail_class == 'PriorityMailInternational': 
				etree.SubElement(root, u'IntegratedFormType').text = 'Form2976A' 
			else:
				etree.SubElement(root, u'IntegratedFormType').text = 'Form2976' 
            #if self.package.mail_class == 'FirstClassMailInternational' else 'Form2976'        
        # added for Parcel Select Barcoded Nonpresorted
        if self.package.mail_class == 'ParcelSelect':
            etree.SubElement(root, u'SortType').text = 'Presorted'
...
#            raise Exception(error_msg)
            raise osv.except_osv(_('Warning !'),_('%s'%(error_msg)))
        else:
