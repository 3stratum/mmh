        ...
        if product_id:
            ...
            if sub_components:
                for each_comp in sub_components:
                    ...
                    comps.append((0, 0,{'qty_uom':each_comp_obj.qty,'name':each_comp_obj.name,'product_id':each_comp_obj.comp_product_id.id,'uom_id':(each_comp_obj.product_id.uom_id.id if each_comp_obj.product_id.uom_id else False),
        ...
        if product:
            ...
            if sub_components:
                for each_comp in sub_components:
                    ...
                    comps.append((0,0,{'qty_uom':com_qty,'name':each_comp_obj.name,'product_id':each_comp_obj.comp_product_id.id,'uom_id':(each_comp_obj.product_id.uom_id.id if each_comp_obj.product_id.uom_id else False),
                    'price':comp_price}))
            if context.get('bundle_configuration'):
                product_obj = self.pool.get('product.product')
                for comp in context.get('bundle_configuration'):
                    ...
                    if product_id:
                        ...
                        comps.append((0,0,{'qty_uom':com_qty,'name':comp.get('name',''),'product_id':product_id[0],'uom_id':(product_id_obj.uom_id.id if product_id_obj.uom_id else False),
                        'price': comp_price}))
            if res.get('value'):
                res['value']['sub_components'] = comps
        return res
