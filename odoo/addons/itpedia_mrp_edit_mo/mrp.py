...
class mrp_production(osv.Model):
    ...
    def _check_boolean(self, cr, uid, ids, field_name, args, context={}):
        ...
        return res
    def _check_partially(self, cr, uid, ids, field_name, args, context={}):
        res = {}
        for production in self.browse(cr, uid, ids, context=context):
            ...
            if moves:
                bool = True
                for move in moves:
                    bool = bool and False if move.state=='cancel' else True
                res[production.id] = bool
            else:
                res[production.id] = False

        return res
    def _check_produced(self, cr, uid, ids, field_name, args, context={}):
        res = {}
        for production in self.browse(cr, uid, ids, context=context):
    ...
    _columns = {
        'consumed': fields.function(
            ...
            help="indicates if product to consume have been consumed\
                or canceled"),
        'partial_consumed': fields.function(
                _check_partially, string='Partially Consumed?',
                type='boolean',
                help="indicates if product to consume have been partially consumed\
                    or canceled"),
        'product_produced': fields.function(
                _check_produced, string='Produced',
                type='boolean',
                help="indicates if product to produce have been partially produced\
                    or canceled"),
